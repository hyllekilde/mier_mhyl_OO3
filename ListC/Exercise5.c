...
void markPhase(int s[], int sp) {
  ...
  //Go over heap and paint until there is no grey elements
  int greys = 1;
  while(greys == 1){
    greys = 0;
    word* heapPtr = heap;
    while(heapPtr<afterHeap){
      //Don't work on NULL and int's
      if(heapPtr[0] == 0 || IsInt(heapPtr[0])){
        heapPtr++;
        continue;
      } 

      if(heapPtr[0] != 0 && Color(heapPtr[0]) == Grey){
        greys = 1;
        mark(heapPtr);
      }
      heapPtr++; //Iterate by going to next element
    }
  }
}

//Mark this element black and its (eventual) references grey
void mark(word* block){
  block[0] = Paint(block[0], Black);
  int i;
  for(i=1; i<=Length(block[0]); i++){
    if(!IsInt(block[i]) && block[i] != 0 && Color(block[i]) == White){
      block[i] = Paint(block[i],Grey);
    }
  }
}
